...
const verifyEip155MessageSignature = (message, signature, address) =>
  verifyMessage(message, signature).toLowerCase() === address.toLowerCase();

export const signMessage = async ({ web3Provider, method }) => {
  if (!web3Provider) {
...
export const ethSign = async ({ web3Provider, method }) => {
  if (!web3Provider) {
    throw new Error("web3Provider not connected");
  }
  const msg = "hello world"; // Message to sign
...
export const signTypedData = async ({ web3Provider, method }) => {
  if (!web3Provider) {
    throw new Error("web3Provider not connected");
  }
...
export const signTransaction = async ({ web3Provider, method }) => {
  if (!web3Provider) {
    throw new Error("web3Provider not connected");
  }
  const [address] = await web3Provider.listAccounts();
